...
def build(published, out_dir, icon):
    ...
    for entry in sorted(os.listdir(published)):
        src = os.path.join(published, entry)
...
def main():
    ...
    ap.add_argument("published", help="directory from `dotnet publish -r osx-*`")
    ap.add_argument("--arch", required=True,
                    help="label for the archive name, e.g. apple-silicon or intel")
    ...
    if not os.path.isdir(args.published):
        raise SystemExit(f"not a directory: {args.published}")
    if not os.path.isfile(args.icon):
        raise SystemExit(f"no icon at {args.icon} - run 'python make_icon.py' first")

    out_dir = args.out or os.path.dirname(os.path.abspath(args.published))
